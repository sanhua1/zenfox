...
find_firefox() {
  local app
  if [[ -n "${ZENFOX_FIREFOX_APP:-}" ]]; then
    app="$ZENFOX_FIREFOX_APP"
    [[ -x "$app/Contents/MacOS/firefox" ]] || die "ZENFOX_FIREFOX_APP is not a Firefox app: $app"
    printf '%s\n' "$app"
    return
  fi
  for app in "/Applications/Firefox.app" "$HOME/Applications/Firefox.app"; do
    if [[ -x "$app/Contents/MacOS/firefox" ]]; then
      printf '%s\n' "$app"
      return
    fi
  done
...
resolve_profile() {
  local base="$HOME/Library/Application Support/Firefox"
  local rel=""
  local candidate=""
  local profile=""
  ...
  candidate=$(ls -td "$base"/Profiles/* 2>/dev/null | head -n 1 || true)
...
FIREFOX_APP=$(find_firefox) || {
  if [[ "$CHECK_ONLY" -ne 1 ]]; then open "https://www.mozilla.org/firefox/new/" >/dev/null 2>&1 || true; fi
  die "Firefox was not found. Install it, launch it once, then rerun Zenfox."
}
FIREFOX_ROOT="$FIREFOX_APP/Contents/Resources"
PROFILE=$(resolve_profile) || die "No Firefox profile found. Launch Firefox once, close it, then rerun Zenfox."

log "Firefox: $FIREFOX_APP"
log "Profile: $PROFILE"
...
if sidebery_installed "$PROFILE"; then
  ...
  if [[ "$ALLOW_MISSING_SIDEBERY" -ne 1 ]]; then
    if [[ "$CHECK_ONLY" -ne 1 ]]; then open "$SIDEBERY_URL" >/dev/null 2>&1 || true; fi
...
if pgrep -f '/Firefox.app/Contents/MacOS/firefox' >/dev/null 2>&1; then
  confirm "Firefox is running. Close it gracefully and continue?" || die "Close Firefox and rerun Zenfox."
  osascript -e 'tell application "Firefox" to quit' >/dev/null
  for _ in $(seq 1 40); do
    pgrep -f '/Firefox.app/Contents/MacOS/firefox' >/dev/null 2>&1 || break
    sleep 0.25
  done
  pgrep -f '/Firefox.app/Contents/MacOS/firefox' >/dev/null 2>&1 && die "Firefox did not exit."
fi
...
[[ -d "$PROFILE/chrome/settings" ]] && cp -R "$PROFILE/chrome/settings" "$BACKUP/profile/chrome/settings"
...
mkdir -p "$PROFILE/chrome/JS" "$PROFILE/chrome/utils" "$PROFILE/chrome/settings"
...
cp -R "$PAYLOAD/profile/chrome/settings/." "$PROFILE/chrome/settings/"
...
CACHE="$HOME/Library/Caches/Firefox/Profiles/$(basename "$PROFILE")/startupCache"
if [[ -d "$CACHE" ]]; then
  mv "$CACHE" "${CACHE}.pre-zenfox-$STAMP"
fi
