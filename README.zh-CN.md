...
在 Zenfox 左上角区域右键，选择 **ZenFox 设置**。Firefox 会打开独立设置标签页。左上角窗口控制组固定在第一行最左侧，其余原生按钮可以在第一行和第二行之间拖拽、排序和隐藏，也可以恢复默认布局。地址栏和扩展行不受该设置影响。

双行配置保存在 Firefox 首选项 `zenfox.toolbarRows.v1` 中，重启浏览器后仍然有效。首次升级会沿用原来的 `zenfox.quickActions.v1` 第二行配置。扩展按钮、地址栏和弹性空白不会出现在候选列表中。
